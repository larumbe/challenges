...
def subsetsumoneliner(lnum):
    return True if ((0 in lnum) or len([(x,-x) for x in lnum if -x in lnum]) > 0) else False

# Bonus
# Today's basic challenge is a simplified version of the subset sum
# problem. The bonus is to solve the full subset sum problem. Given a
# sorted list of distinct integers, write a function that returns
# whether there is any non-empty subset of the integers in the list
# that adds up to 0.

# Examples of subsets that add up to 0 include:

# [0] [-3, 1, 2] [-98634, -86888, -48841, -40483, 2612, 9225, 17848,
# 71967, 84319, 88875]

# So if any of these appeared within your input, you would return
# true.

# If you decide to attempt this optional challenge, please be aware
# that the subset sum problem is NP-complete. This means that's it's
# extremely unlikely that you'll be able to write a solution that
# works efficiently for large inputs. If it works for small inputs (20
# items or so) that's certainly good enough.
...
# # Naive exponential way
def subsetsum(add,lnum):
    for s in [list(x) for x in list(itertools.chain.from_iterable(itertools.combinations(lnum, r) for r in range(1,len(lnum)+1)))]:
        if sum(s) == add:
            return True
    return False

def subsetsumonline(add,lnum):
    return len([ss for ss in [list(x) for x in list(itertools.chain.from_iterable(itertools.combinations(lnum, r) for r in range(1,len(lnum)+1)))] if sum(ss) == add])p > 0
